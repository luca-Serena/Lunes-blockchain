...
//the index of the head in the greater position is returned
...
//The smallest fork is replaced with a new one
...
void lunes_user_control_handler(hash_node_t *node) {
    ...
    if ((int) simclock % INTERMEDIATE_STEPS == 0) {
	    ...
	    if (node->data->miner) {
	        ...
	        if (p > random_mine && node->data->internal_timer > 2) {
		        ...
		        if (ltblock < 2500 - 1){    //with rispect to the non-forking version the blockchain size is increased, due to foking version producing more blocks
		            ...
		            if ( headPos >= 0){                                               //adding the block to the longest chain
                        ...
                        Block * thatBlock = heads[headPos];                           //head block of the longest chain
		               	b->prevId = thatBlock->id;
		               	b->position = thatBlock->position + 1;
		            	...
		            	b->prevId = -1;                                               //first block of a chain has -1 as previous block index and 1 as position
		                b-> position = 1;
		                replace_heads (node->data->s_state.heads, 0, b);
		            }
	            ...
	            if (node->data->attackerid == node->data->key && selfish[2] >= 0) {
	                ...
	                selfish[2] += 1;
	                if (selfish[2] >= 2) {
	                    //Head block and his previous are spread through the network. 
	                    #ifdef FORKING     //in case of forking the indexes have to be found
	    ...
	    if (((node->data->s_state.time_of_next_trans >= 0) &&
	         (node->data->s_state.time_of_next_trans <= simclock)) || node->data->key == 1337) {
    ...
    if (msg->trans.trans_static.ttl > 0) {
        ...
        if (node->data->s_state.freerider == 0) {
            ...
            if (node->data->key != msg->trans.trans_static.creator &&
                lunes_trans_is_known(node->data->s_state.blockchain, node->data->latestblock, msg->trans.trans_stMsgatic.transid) == 0) {
    ...
    if (msg->block.block_static.ttl > 0) {
        ...
        if (node->data->s_state.freerider == 0) {
            ...
            if (node->data->attackerid == node->data->key && selfish[2] >= 0) {
                ...
                selfish_1_pos = pos > selfish_1_pos || selfish[1] <= 0 ? pos : selfish_1_pos;
                int diff_prev =  selfish_0_pos - selfish_1_pos;
                if (diff_prev == 0) {
                    selfish[2] = 0;             //zero blocks in advantage
                } else if (diff_prev == 1) {
         ...
         //                                                    recover previous block is disconnected from the network for 200 time steps
            ...
            if (is_in_blockchain(&node->data->s_state.blockchain[0], receivedBlock->id, ltblock) == -1){  //if it's a new block
	            ...
	            if (receivedBlock->position == 1){                    //if it's the first block in the chain
	            	add_heads(node->data->s_state.heads, receivedBlock);        	
	            } else {
	            	...
	            	if (pos != -1){                                    //if previous block is a head block
	            		replace_heads (heads, pos, receivedBlock);
	            	} else {                                           //if the previous block not a head block but it's in the blockchain            	    
	            	    if (is_in_blockchain(&node->data->s_state.blockchain[0], prevId, ltblock)!= -1)	{   
	            		    int next = is_next_in_blockchain(&node->data->s_state.blockchain[0], node->data->latestblock, receivedBlock->id);
	            		    while (next != -1){  //the head of that fork
	            	    ...
	            	    } else {             // previous node not in blockchain 
	            	    	//ask for the minor position between the position of the received block and the position of the longest head block
                            int positionToAsk = 1;
                            if (heads_greater_position(heads) != -1){
                                positionToAsk = heads[heads_greater_position(heads)]->position + 1;
                            }
	            	    	if (receivedBlock->position - 1 < positionToAsk ){   
	                            positionToAsk = receivedBlock->position == 1 ? 1 : receivedBlock->position -1;
	            	    	}
			        	    GHashTableIter iter;
			                gpointer       key, destination;
			                hash_node_t *  sender, *receiver; // Sender and receiver nodes in the global hashtable
